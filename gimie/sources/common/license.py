...
def get_branch_name_and_file_dict(repo_url):
    ...
    if files_dict:
        for file in files_dict:
            ...
            if re.match(pattern, file["name"], flags=re.IGNORECASE):
                license_path = (
                    repo_url
                    + f"/blob/{get_branch_name_and_file_dict(repo_url)[0]}/"
                    + file["name"]
                )
...
def get_license(repo_url, headers, license_files=[]):
    url = get_license_path(
        get_branch_name_and_file_dict(repo_url)[1], license_files=license_files
    )
